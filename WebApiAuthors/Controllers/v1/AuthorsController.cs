...
namespace WebApiAuthors.Controllers.v1
{
    ...
    public class AuthorsController : ControllerBase
    {
        ...
        [AllowAnonymous]
        public async Task<ActionResult<AuthorDTO>> Get([FromQuery] PaginationDTO pagintionDTO)
        {
            var queryable = context.Authors.AsQueryable();
            await HttpContext.InsertPaginationParametersInHeader(queryable);

            var author = await queryable.OrderBy(author => author.Name).Paged(pagintionDTO).ToListAsync();
        ...
        [HttpGet("{searchedName}", Name = "GetAuthorByName")]
